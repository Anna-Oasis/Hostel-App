...
import { getAlterRCLeave, updateAlterRCLeaveStatus } from "@/utils/rc/rcAlterLeave";

export default function AlterRcLeavePage() {
  type RcLeaveApiItem = {
    leave: {
      id: number;
      rc_id: number;
  ...
  const fetchLeaves = async () => {
    try {
      setLoading(true);
      const result = await getAlterRCLeave();
      if (result.success && Array.isArray(result.data)) {
        setLeaves(result.data);
      }
    } catch (err: any) {
      window.alert(err?.message || "Error fetching RC leaves");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApprove = async (leaveId: number) => {
    try {
      const result = await updateAlterRCLeaveStatus(leaveId, "true");
      if (result.success) {
        await fetchLeaves();
        setModal({
          show: true,
  ...
  const handleReject = async (leaveId: number, comment: string) => {
    try {
      const result = await updateAlterRCLeaveStatus(leaveId, "false", comment);
      if (result.success) {
        setLeaves((prev) =>
          prev.map((leave) =>
  ...
  return (
    <>
      ...
      <DeclineComment
        ...
        title="Reject Alternate RC Leave"
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        {leaves.length === 0 ? (
          <EmptyPage
            title="No Alternate RC Leave requests found."
            description="There are currently no Alternate RC Leave requests to review."
          />
        ) : (
          leaves.map((item) => (
            <ApprovalCard
